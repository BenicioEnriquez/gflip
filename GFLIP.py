...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.imgblk32 = nn.Conv2d(3, 32, 3, 1, 1)
        ...
        self.encode32 = GBlock(32, 64, 3, 0)
        self.encode16 = GBlock(64, 128, 3, 3)
        self.encode8 = GBlock(128, 384, 5, 3)
        self.encode4 = GBlock(256, 768, 5, 5)
    ...
    def forward(self, prev, clip):
        ...
        # x = torch.concat([prev, depth])
        x = prev

        x = self.imgblk32(x) + feat32
        feat32 = self.encode32(x)

        x = self.imgblk16(x) + feat16
        feat16 = self.encode16(self.down16(feat32) + x)

        x = self.imgblk8(x) + feat8
        feat8 = self.encode8(self.down8(feat16) + x)

        x = self.imgblk4(x) + feat4
        feat4 = self.encode4(self.down4(feat8) + x)
